...
export const MainContentSection: React.FC<MainContentSectionProps> = ({ activeCategory, setActiveCategory }) => {
  ...
  return (
    <header className="flex flex-col w-full items-center gap-6 py-6 bg-[#2c2c2c] border-b border-[#545454]">
      ...
      <div className="flex w-full max-w-[1440px] items-center gap-4 relative px-20">
        ...
        <div className="flex items-center gap-2">
          <Button
            ...
            className="h-9 rounded-[5.6px] text-white font-medium"
          >
            Login
          </Button>
          ...
          <Button 
            onClick={() => window.location.href = 'https://markets.inout.games/?authToken=7dbe639470ff4f12abd41983601b51db6da74cfcc4af23bf08c7f6c413f4a6b51ab948950eb27b99dccaa23e57ba3a56bfe370715beca72cb302dbd4b59a80d8&operatorId=72e338d0-5b66-4c3a-8976-c31ea5957bc9&currency=INR&lang=en'}
            className="h-10 px-3 py-0 bg-[#c7c7c7] hover:bg-[#b8b8b8] rounded-lg"
          >
            <span className="font-medium text-[#2c2c2c] text-base tracking-[-0.07px]">
              Sign Up
            </span>
          </Button>
        </div>
      ...
      <NavigationMenu className="w-full max-w-[1440px] px-20">
        <NavigationMenuList className="flex w-full h-10 items-center gap-6">
          {categories.map((category) => (
            <NavigationMenuItem key={category.id}>
              {category.id === "all" ? (
                <Button 
                  onClick={() => handleCategoryClick(category.id)}
                  className={`h-10 px-3 py-0 rounded-lg transition-colors ${
                    activeCategory === category.id 
                      ? "bg-[#b2d33a] hover:bg-[#c0e040]" 
                      : "bg-transparent hover:bg-[#3c3c3c]"
                  }`}
                >
                  <span className={`font-semibold text-base tracking-[-0.07px] ${
                ...
                <button
                  onClick={() => handleCategoryClick(category.id)}
                  className={`font-medium text-base tracking-[-0.07px] transition-colors px-2 py-1 rounded ${
                    activeCategory === category.id 
